...
world_map <- ne_countries(type = "countries", returnclass = "sf", scale = 110) %>% st_transform(crs = 3857)  # Spherical/Web Mercator projection
...
## Creating the cartograms

Now we'll create the cartograms
...
world_map_forest_area_2020_projected <- st_transform(drop_na(world_map_forest_area_2020, "Forest area") , crs = 3857)

st_crs(world_map_forest_area_2020)

cartogram_continuous111 <- cartogram_cont(world_map_forest_area_2020_projected, 
                                       "Forest area", 
                                       itermax = 5)

cartogram_continuous2 <- cartogram::cartogram_dorling(
  world_map_forest_area_20201, weight = "Forest area", k = 5,
  m_weight = 1, itermax = 1000)

ggplot(cartogram_continuous111) + 
  geom_sf(aes(fill = `Forest area`)) + 
  coord_sf(crs = 4326) +  # Robinson projection
  theme_bw()
