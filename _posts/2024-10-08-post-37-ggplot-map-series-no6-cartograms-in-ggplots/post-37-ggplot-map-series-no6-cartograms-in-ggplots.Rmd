...
### Compute the discrete cartogram (Dorling cartogram)
cartogram_discrete <- cartogram_dorling(world_map_forest_area_2020, 
                                        weight = "Forest area",
                                        itermax = 100)
...
p_discrete_cartogram <- ggplot() + 
  geom_sf(data = world_map_forest_area_2020, fill = "#2ca25f", color = alpha("#b2e2e2", 0.5)) + 
  geom_sf(data = cartogram_discrete, fill = alpha("#dfc27d", 0.8), color = alpha("#a6611a", 0.8)) + 
  labs(title = "Discrete cartogram (Dorling cartogram)") + 
  theme_void() + 
  theme(plot.title = element_text(hjust = 0.5, vjust = 3, size = 14, color = "grey"),
        plot.background = element_rect(fill = "#045a8d", color = NA),
        plot.margin = margin(t = 15))

p_continuous_cartogram / plot_spacer() / p_discrete_cartogram + 
...
world_map_percent_forest_area_2020 <- world_map_forest_area_2020 %>% 
...
### Create pie charts
world_map_percent_forest_area_2020_pies <- world_map_percent_forest_area_2020 %>% 
  mutate(pie_chart = map(percent_forest_area, function(percent_forest_area){
    p_grob <- ggplotGrob(ggplot() + 
      ...
      # geom_vline(xintercept = 0.5, linewidth = 0.1, color = alpha("#a6611a", 0.8)) + 
      # geom_vline(xintercept = 1.5, linewidth = 0.1, color = alpha("#a6611a", 0.8)) + 
...
### Get the country centroids from the Dorling cartogram circles
country_centroids <- cartogram_discrete %>% 
  mutate(country_centroid_lon = st_coordinates(st_centroid(cartogram_discrete)$geometry)[, 1],
         country_centroid_lat = st_coordinates(st_centroid(cartogram_discrete)$geometry)[, 2]) %>%
  select(Code, country_centroid_lon, country_centroid_lat) %>% 
  st_drop_geometry()

world_map_percent_forest_area_2020_pies <- world_map_percent_forest_area_2020_pies %>% 
  inner_join(., country_centroids, by = join_by("Code"))

### Add the pie charts to the discrete cartogram

cartogram_dorling %>%
  mutate(
    # Compute area
    ar = as.numeric(st_area(cartogram_dorling)),
...
ggplot() + 
    geom_sf(data = world_map_percent_forest_area_2020_pies, fill = "#2ca25f", color = alpha("#b2e2e2", 0.5)) + 
    geom_sf(data = cartogram_discrete, fill = alpha("#dfc27d", 0.8), color = alpha("#a6611a", 0.8)) + 
    theme_void() + 
    theme(plot.title = element_text(hjust = 0.5, vjust = 3, size = 14, color = "grey"),
          plot.background = element_rect(fill = "#045a8d", color = NA)) + 
    annotation_custom(world_map_percent_forest_area_2020_pies$pie_chart[[3]],
                        xmin = world_map_percent_forest_area_2020_pies$country_centroid_lon[[3]] - 1510493*1.2,
                        xmax = world_map_percent_forest_area_2020_pies$country_centroid_lon[[3]] + 1510493*1.2,
                        ymin = world_map_percent_forest_area_2020_pies$country_centroid_lat[[3]] - 1510493*1.2,
                        ymax = world_map_percent_forest_area_2020_pies$country_centroid_lat[[3]] + 1510493*1.2)
  

### 
reduce(1:nrow(world_map_percent_forest_area_2020_pies), .f = function(x, y){
  x + annotation_custom(world_map_percent_forest_area_2020_pies$pie_chart[[y]],
                        xmin = world_map_percent_forest_area_2020_pies$country_centroid_lon[[y]] - 500000,
                        xmax = world_map_percent_forest_area_2020_pies$country_centroid_lon[[y]] + 500000,
                        ymin = world_map_percent_forest_area_2020_pies$country_centroid_lat[[y]] - 500000,
                        ymax = world_map_percent_forest_area_2020_pies$country_centroid_lat[[y]] + 500000)
  }, 
  .init = ggplot() + 
    geom_sf(data = world_map_percent_forest_area_2020_pies, fill = "#2ca25f", color = alpha("#b2e2e2", 0.5)) + 
    geom_sf(data = cartogram_discrete, fill = alpha("#dfc27d", 0.8), color = alpha("#a6611a", 0.8)) + 
...
ggplot() + 
      geom_col(aes(x = 1, y = 55), fill = alpha("#a6611a", 0.8), width = 1) + 
      # geom_vline(xintercept = 0.5, linewidth = 0.1, color = alpha("#a6611a", 0.8)) + 
      # geom_vline(xintercept = 1.5, linewidth = 0.1, color = alpha("#a6611a", 0.8)) + 
      scale_y_continuous(limits = c(0, 100), expand = c(0, 0)) + 
      scale_x_continuous(limits = c(0.5, 1.5), expand = c(0, 0)) + 
