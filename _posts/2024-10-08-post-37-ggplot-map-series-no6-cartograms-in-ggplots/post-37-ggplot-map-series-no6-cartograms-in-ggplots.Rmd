...
In this final section, we'll create a variant of Dorling cartogram by adding pie charts of the percent forest cover in the countries. This involves four steps:
...
percent_forest_area_2020_pie_charts <- percent_forest_area_2020 %>% 
  mutate(pie_chart = map(percent_forest_area, function(percent_forest_area){
    p_grob <- ggplotGrob(ggplot() +  # convert the ggplot objects into grobs
...
3. Third, we got the centroids and areas of the circles in the Dorling diagram using `st_centroid()` and `st_area()` and calculated the radii of the circles. These were used for drawing the pie charts.

<div style="height: -1em;"></div>

```{r, fig.width = 7, fig.height = 4.5, out.width = "80%"}
### Step 3. Get the centroids and radii of the country circles from the Dorling cartogram
...
4. Finally, we added the pie charts to the dorling cartogram using the function `annotation_custom()`. The radii of the pies were multiplied with a scalar (in this example 1.2) so that the pies fit to the circles. The function `reduce()` allowed us to iterate over the rows to add the pie chart of each country based on its centroid and radius.
