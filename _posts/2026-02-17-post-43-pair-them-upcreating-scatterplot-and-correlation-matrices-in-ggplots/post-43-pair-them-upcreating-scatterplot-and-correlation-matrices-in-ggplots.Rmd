...
There are two more things we can do to make the heatmap even fancier: clustering the variables based on the correlation matrix and annotating the plot. 


To annotate the heatmap, we'll first create a new dataframe with a column indicating the variable names and one or more columns of annotation variables (which can be both categorical or continuous). Simply pass the data to the argument "annot_rows_df = " (or "annot_cols_df = " depending on which side of the heatmap you would like annotate) and the annotation will show up in the plot! We can further customize the annotation using a variety of the "annot_XXX" arguments, as shown in the example below. 
...
ggcorrhm(Soils_continuous, 
         ...
         show_dend_cols = F,  # hide the column dendrogram 
         ...
         annot_dist = 0.3,  # distance between the annotation and the heatmap
