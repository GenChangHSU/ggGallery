...
```{r, fig.width = 10, fig.height = 5, out.width = "80%"}
library(MASS)
kd <- kde2d(iris$Sepal.Length, iris$Sepal.Width, n = 100)[[3]]

surfaceplot3d <- plot_ly() %>%
  add_surface(x = seq(min(iris$Sepal.Length), max(iris$Sepal.Length), length.out = 100),
              y = seq(min(iris$Sepal.Width), max(iris$Sepal.Width), length.out = 100),
              z = ~ kd,
  ...
  layout(legend = list(x = 0.5, y = 0.1, orientation = 'h'),
         scene = list(xaxis = list(title = "Xaa"),
                      yaxis = list(title = "Y"),
                      zaxis = list(title = "Z")
                      )
         )
