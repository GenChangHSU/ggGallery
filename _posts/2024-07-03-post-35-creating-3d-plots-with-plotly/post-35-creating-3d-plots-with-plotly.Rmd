...
In this example, we created a contour plot in ggplot and pass the ggplot object to `plot_gg()`. There are many additional arguments you can specify for the 3D plot: the width and height of the plot, the height of the z-axis, the shadow intensity, the isometric field of view angle, the zoom level, the angle of rotation around the z-axis, etc. The 3D plot will be displayed in a pop-up window. To save the plot as a .png file, simply run `rander_snapshot(filename = )`.
...
To recap, we explored two packages for 3D data visualization in R. The first one is `plotly`, which creates a variety of interactive plots, and we made a 3D scatterplot and a 3D contour plot with it. The second one is `rayshader`, which creates stunning 3D elevation maps. It can also convert a 2D ggplot to a 3D plot, and we did that using examples of a contour plot and a heatmap. There are a lot more different kinds of 3D plots you can make with these two packages—the sky's the limit, and we only scratched the surface here. But at least now you know the basics and you can go on and explore more!
