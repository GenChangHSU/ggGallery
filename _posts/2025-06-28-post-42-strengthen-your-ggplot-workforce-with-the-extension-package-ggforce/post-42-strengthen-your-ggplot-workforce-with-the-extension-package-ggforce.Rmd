...
<span style="display: block; margin-top: 0px; margin-left: 10px">(1) `geom_parallel_sets()`, which creates parallel sets/alluvial plots from a set of categorical variables. Basically, we need a dataframe with the counts of all unique combinations of the levels in the categorical variables. We then pass this dataframe to `gather_set_data()` to convert it into a format used by `geom_parallel_sets()`. Two companion functions `geom_parallel_sets_axes()` and `geom_parallel_sets_labels()` allow users to customize the line sets (i.e., the segments along the y-axis representing the levels of each categorical variable) and their labels.</span>
...
ggplot(geom_parallel_sets_dat) +
  ...
  theme_void() + 
  theme(axis.text.x = element_text(color = "black", margin = margin(t = -5, b = 5)))
