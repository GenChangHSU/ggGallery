...
We can spice up the "plain" map by adding points/lines/polygons to it. Here, I'm going to draw the boundary of Ithaca City and show the locations of the city street trees. The shapefiles were downloaded from [the city website](). We'll read in these files using the function `st_read()` from the package `sf` (see my [previous post]() on making maps with `sf`!) and add them to the map using `geom_sf()`. Note that we need to set "inherit.aes = F" so that `geom_sf()` will not use the aesthetics from the global plot (which is used for the satellite map).
...
city_boundary <- st_read("./Municipal_Boundary/Municipal_Boundary.shp")
city_trees <- st_read("./City_Managed_Trees/City_Managed_Trees.shp")

### Add the city boundary and trees to the satellite map
get_googlemap(my_location, zoom = 13, maptype = "satellite") %>% 
  ggmap() + 
  geom_sf(data = city_boundary, fill = NA, color = "red", inherit.aes = F) +  # need to set "inherit.aes = F"
  geom_sf(data = city_trees, color = "forestgreen", inherit.aes = F, size = 0.01)
