...
This is quite a long journey, and so I'll break it down into several legs so that we can fully explore the great scenery of the outer space along the way.
...
```{r, fig.width = 9, fig.height = 9, out.width = "80%", eval = F}
library(ggtext)
...
We're almost done! Our final leg is to embellish the figure with some glittering stars. We'll do this by first creating another ggplot with random blurry points scattered across a transparent background, and then overlaying it on the original planet figure using the functions in the package `cowplot`.

```{r, fig.width = 9, fig.height = 9, out.width = "80%"}
set.seed(123)
stars_df <- data.frame(x = runif(50),
                       y = runif(50),
                       size = runif(50))  # stars of random positions and sizes
...
P_final <- ggdraw(P_solar_system) + 
  draw_plot(P_stars)  # overlay the stars on the planet figure

P_final

```

Well done! We make it to the very end of the journey. This is absolutely an amazing journey. Unfortunately, our rocket is running out of fuel and so it's time to go back to the Earth. Hope you enjoy this memorable trip and will see you soon next time!
