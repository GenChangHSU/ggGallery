...
P_solar_system <- P_solar_system + 
  ...
  geom_richtext(data = planetary_dat, aes(x = 3.5, y = 17.5,
...
```{r, fig.width = 9, fig.height = 9, out.width = "80%"}
stars_df <- data.frame(x = runif(100, min = -5, max = 12),
                       y = runif(100, min = -10, max = 100),
                       size = runif(100, min = 0, max = 1))

P_solar_system <- P_solar_system + 
  geom_point_blur(data = stars_df, aes(x = x, y = y, size = size),
                  color = "white", position = position_nudge(x = 5)) + 
  scale_blur_size_continuous(range = c(0, 0.1))
...
Hope you enjoy the memorable journey and don't forget to leave your comments and suggestions below if you have any!
