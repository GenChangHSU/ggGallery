...
The previous two legs were mostly data preparation, and now we are ready to roll up our sleeves and do some heavy lifting. In our third leg, we will create a polar line-dot chart for the planets, with the length of the line/curve proportional to the distance of the planet to the Sun, and the size of the point proportional to the volume of the planet.

Here I used the function `geom_point_blur()` from the package `ggblur` to add blurry points, and the functions `lighten()` and `desaturate()` to modify the color of the points and lines. Also, I extended the lower limit of the x-axis to -1 to create some space at the center of the figure where we will be adding an image later, and extended the upper limit of the y-axis to 35 to prevent the longest line from sticking too close to the origin.  
...
P_solar_system <- P_solar_system + 
  ...
  geom_richtext(data = planetary_dat, aes(x = 3.3, y = 17.5,
                                          label = "<b><span style = 'font-size:25pt'>A</span>n<span> </span><span style = 'font-size:25pt'>E</span>pic<span> </span><span style = 'font-size:25pt'>J</span>ourney<span> </span>to<span> </span>the<span> </span><span style = 'font-size:25pt'>S</span>olar<span> </span><span style = 'font-size:25pt'>S</span>ystem</b><br>"),
                size = 5, 
                color = NA,
                fill = NA,
                text.color = "#09bab7",
