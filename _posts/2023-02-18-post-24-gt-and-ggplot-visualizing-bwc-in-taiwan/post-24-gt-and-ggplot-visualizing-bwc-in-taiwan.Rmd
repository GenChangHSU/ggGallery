...
bwc_table <- bwc_top_collision_df %>% 
  ...
  text_transform(locations = cells_body(columns = linechart),
                 fn = function(x){map(bwc_linecharts, ggplot_image, 
                                      height = px(90), aspect_ratio = 2.2)}) %>%   # header
...
The table has its shape, but definitely needs some adjustments and fine-tuning. This is what we are going to do next.
