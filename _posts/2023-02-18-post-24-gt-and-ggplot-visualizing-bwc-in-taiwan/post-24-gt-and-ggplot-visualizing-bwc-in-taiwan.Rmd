...
Nice, we get our table here! Though, it is not in a good shape and definitely needs some adjustments and fine-tuning. This is what we are going to do next.
...
Looks cool, but still there are a bit of formatting issues (e.g., the short descriptions are cut off on the right) when displayed as an HTML page here.
...
In general, the collisions for these top ten species are trending upwards, despite some year-to-year fluctuations. This may be due to more reports in recent years as people are more aware of bird-window collisions. It can also be because there are more buildings/infrastructures with window surface, thus causing more collisions. The increasing trends are concerning and deserve our close attention.

Many of the bird species on the list commonly occur in urban or sub-urban areas (e.g., spotted dove, Eurasian tree sparrow, light-vented bulbul, Swinhoe's white-eye, and oriental turtle dove), which is a strong reason why they collide with windows quite frequently. But the collision issue is not limited to these urban residents. There is also common kingfisher, a species that lives around water bodies, and pale thrush, a winter migrant in Taiwan.

<!-- The first species, Taiwan barbet, almost double the number of the second species! It is an endemic species in Taiwan. It is quite unexpected that this species tops the list as not many cases have been reported for similar species in other countries. -->

<!-- Another surprising species on the list is the raptor species crested goshawk. It's a bit mind-boggling that such an agile flyer also falls victim to bird-window collision. This could potentially have some ecological impacts as hawks are top predators in the ecosystem. -->

<!-- Interestingly, there are three dove species on the list. This may be due to the fact that they are relatively poor in maneuvering. But it can also simply be that they're quite abundant around human settlements. Or perhaps a combination of them. -->

<!-- It is noteworthy that the numbers shown in the table are just the tip of the iceberg: a lot of bird-window collisions got unnoticed, or noticed but unreported. There is increasing research on why birds collide with windows, and hopefully we will come up with effective measures to prevent this. -->

## Summary

To recap, in this post we built a cool `gt` table to visualize the top ten bird-window collision species in Taiwan. We first prepared the table columns, then put them in a data frame and created an initial table, and finally polished the table by tweaking the format and style. We also integrated ggplots into the table to enhance data visualization.
