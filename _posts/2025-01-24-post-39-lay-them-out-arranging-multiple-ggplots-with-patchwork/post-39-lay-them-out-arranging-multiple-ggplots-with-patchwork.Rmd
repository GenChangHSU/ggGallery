...
Sometimes, we have a set of ggplots we would like to arrange and they are stored in a list. We can simply pass the entire list to the function `wrap_plot()`, which will pack all the plots in the list. This is essentially the same as "p1 + p2 + ... + pn", but it saves us the time of having to pull the individual plots out and add "`+`" between them.
