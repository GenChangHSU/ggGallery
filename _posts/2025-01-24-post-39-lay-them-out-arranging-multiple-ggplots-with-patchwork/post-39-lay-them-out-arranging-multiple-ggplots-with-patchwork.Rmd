...
```
We can also create a designated area to place the legend(s) using `guide_area()`:
...
Similar to legends, for plots that share the same axes, we can 

```{r, fig.width = 5, fig.height = 4}
### Create a designated area for the legend(s)
p1 + p3 + p2 + plot_layout(axes = "collect")
p1 + p3 + p2 + plot_layout(axis_titles = "collect")

```



### Text annotation (title, subtitle, tags, axis text)
