...
We'll split the original dataframe into individual subsets by journal and do the computation for each of them. For each subset, we'll then create four new columns: **x_from**, **x_to**, **y_from**, and **y_to**. **x_from** is the current year plus the bar width (which is 0.05); **x_to** is the next year minus bar width; **y_from** is the upper/lower ends of the bars in the current year; **y_to** is the upper/lower ends of the bars next year. Together, these four columns will serve as the x- and y-coordinates of the two points (**x_from**, **y_from**) and (**x_to**, **y_to**) that are used for computing a smooth curve.

Note that the **y_to** for the year 2022 was set to `NA` because there is no further bar to go next, and the row for that year was later removed. Also, I subtracted 0.1 from the upper end and added 0.1 to the lower end to create a margin between ribbons so that they will not get stuck to each other.

Next,  I used the function `sigmoid()` from the package `ggbump` to compute a smooth sigmoid curve between the two points (**x_from**, **y_from**) and (**x_to**, **y_to**). The smoothness of the curve can be adjusted using the arguments `n` and `smooth` (play around with different values to see how the final chart changes!).

Finally, the results of each subset were merged and form a single dataframe with all the information we need for plotting the ribbons.
