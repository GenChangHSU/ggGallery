...
Note that the **y_to** for the year 2022 was set to `NA` because there is no next bar to go, and the row for that year was later removed. Also, I subtracted 0.1 from the upper end and added 0.1 to the lower end to create a margin between ribbons so that the ribbons will not go shoulder to shoulder.
...
Finally, the results (the x- and y-coordinate of the points forming the smooth curves) of each subset were merged into a single dataframe, which contains all the information we need for plotting the ribbons.
...
I also used the function `move_layers()` from the package `gginnards` to pull the bars (drawn on the `geom_rect()` layer) to the top. Otherwise, the ribbons will completely cover the bars!
...
Here we are at our final step: polishing the appearance of the chart! Again, we'll add a logo title, a subtitle, and a caption to it. We'll also label the years along the x-axis as well as the journals on the left of the ribbons.  
