...
Creating a basic bump chart is as simple as making a scatterplot (or dotplot) of time vs. rankings and "connecting the dots" with lines, which can be done with `geom_point()` and `geom_line()`. Next, you can use `scale_y_reverse()` to flip the y-axis so that the highest-ranking item stays on the top of the plot. 
...
There are a few things we can do to modify the plot:

<span style="display: block; margin-top: 0px; margin-left: 10px"> (1) Add a title, subtitle, and caption</span>
...
# The journal names on the left side of the figure (using css syntax for element_markdown())
...
ggplot(BES_journals_data) + 
  ...
  labs(title = "<img src='https://raw.githubusercontent.com/GenChangHSU/ggGallery/master/_posts/2022-08-01-post-18-bump-charts-with-ggplot/BES_logo_tp.png' width = '100'/>",  # add the BES logo using the HTML <img> tag
...
Note that I used the package [`ggtext`](https://wilkelab.org/ggtext/) to embed a logo in the plot title with the HTML <img> tag and to label the journal names with the HTML <span> tag. The package has many nice features for text manipulations in ggplots using HTML and CSS syntax. I won't go into the details here as this is not the topic of the post, but do check out my previous [post](https://genchanghsu.github.io/ggGallery/posts/2021-07-10-post-5-awesome-text-display-with-ggtext/) on `ggtext` if interested!

## A modified bump chart with smooth lines
...
ggplot(BES_journals_data) + 
  ...
  geom_point(aes(x = Year, y = Ranking, color = Journal), shape = 21, fill = "white", size = 4, stroke = 2) +
  geom_text(data = data.frame(x = 11.5, y = 1:5), aes(x = x, y = y, label = y)) +
  scale_y_reverse(labels = Journal_label) +  # the highest-ranking journal is on the top
  scale_color_manual(values = col_pal) +  # use the customized color palette
  labs(title = "<img src='https://raw.githubusercontent.com/GenChangHSU/ggGallery/master/_posts/2022-08-01-post-18-bump-charts-with-ggplot/BES_logo_tp.png' width = '100'/>",  # add the BES logo using the HTML <img> tag
       subtitle = "Journal rankings based on their impact factors each year",
       caption = "Data source: Journal Citation Reports") +
