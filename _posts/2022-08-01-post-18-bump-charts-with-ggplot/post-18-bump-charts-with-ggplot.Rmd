...
```{r, fig.width = 9, fig.height = 5, out.width = "85%"}
BES_journals_data_bar <- BES_journals_data %>% 
...
head(BES_journals_data_bar, n = 10)
...
ribbon_bumpchart <- ggplot(BES_journals_data_bar) + 
...
BES_journals_data_ribbon <- BES_journals_data_bar %>% 
  split(., .$Journal) %>% 
  map(., function(data){
    lower_curve <- data %>%
    ...
    curve <- left_join(lower_curve, upper_curve[, -1], by = "x")
  }) %>% bind_rows()

head(BES_journals_data_ribbon, 5)
```
...
ribbon_bumpchart +
  ...
  scale_y_continuous(breaks = filter(BES_journals_data_bar, Year == 2011)$y_end - filter(BES_journals_data_bar, Year == 2011)$IF/2, labels = rev(Journal_label)) +
  labs(title = "<img src='https://raw.githubusercontent.com/GenChangHSU/ggGallery/master/_posts/2022-08-01-post-18-bump-charts-with-ggplot/BES_logo_tp.png' width = '100'/>",
       subtitle = "Journal rankings based on their impact factors each year",
       caption = "Data source: Journal Citation Reports") +
  theme_void() +
