...
<span style="display: block; margin-top: 5px; margin-left: 10px"> (2) Determine the upper and lower curves that connect the bars of each journal</span>
...
### Step 1. Create a stacked barplot of journals' impact factors each year

To create a stacked barplot of journals' impact factors each year, we need to determine the lower and upper end of the bars. and we do this by first arranging the journals in a descending order by year (so the lowest-ranking journal will be the first) and then
computing the cumulative sums of the journals' IF. For each journal, the upper end will be the cumulative sums 

(which will be the upper end of the bar). The cumulative sums of the previous journal. 

It's noteworthy that we do not use the conventional geom layer `geom_bar()` to create the barplot. The main reason to use `geom_rect()` rather than `geom_bar()` is that `geom_rect()` allow us to order the stacks differently for each year, whereas `geom_bar()` will place the stacks in the same order across years.   
...
ribbon_bumpchart <- ggplot(BES_journals_data_barplot) + 
  geom_rect(aes(xmin = as.numeric(Year) - 0.05, xmax = as.numeric(Year) + 0.05,
                ymin = y_start, ymax = y_end, fill = Journal), color = "white") +
