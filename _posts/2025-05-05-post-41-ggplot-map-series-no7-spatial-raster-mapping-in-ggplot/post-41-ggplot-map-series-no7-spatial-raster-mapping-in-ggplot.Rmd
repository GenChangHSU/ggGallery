...
### https://stackoverflow.com/questions/77216826/preserve-axis-labels-in-geom-sf-when-changing-projection
...
# create an sf object for the long-lat lines 
grid_lines <- st_sf(geometry = st_sfc(st_multilinestring(x = append(lon_lines, lat_lines)), crs = 'WGS84'))

# latitude labels


lat_labels <- lapply(c(-90, -60, -30, 0, 30, 60, 90), function(x) {
  st_sf(label = ,
        geometry = st_sfc(st_point(c(-180, x)), crs = 'WGS84'))
}) %>% bind_rows()

lat <- c(-90, -60, -30, 0, 30, 60, 90)
lat_labels_df <- data.frame(label = str_glue("{abs(lat)} °{if_else(lat == 0, '', if_else(lat < 0, 'S', 'N'))}"),
                            lon = -180,
                            lat = lat)

lat_labels <- st_as_sf(lat_labels_df, coords = c("lon", "lat"), crs = "WGS84")
...
### Make the map
ggplot() +
  geom_spatraster(data = january_precipitation_raster_robinson) +  # January precipitation
  geom_sf(data = country_boundary_robinson, fill = NA, color = "black", size = 0.8) + 
    geom_sf(data = grid_lines, linewidth = 0.1, color = "grey") +
  geom_sf_text(data = lat_labels, aes(label = label), size = 3, color = 'gray30',
