...
The second way of handling the problem is to use the function `rename_geom_aes()` in the package `relayer`. The basic idea of the function is to assign different "names" to the same aesthetic (e.g., color) as if they were different aesthetics so that these "pseudo-aesthetics" would be treated independently. 
...
ggplot(iris) +
  ...
  # 1. use the new name ("colour_for_species") for the colour aesthetic in the geom layer
  # 2. tell ggplot that "colour_for_species" is actually a color aesthetic in rename_geom_aes()
  ...
  # the color scale for the first geom layer (mapped to "color")
  scale_color_viridis_c(aesthetics = "color", name = "Sepal length") + 
  
  # the color scale for the second geom layer (mapped to "colour_for_species")
...
ggplot(iris, aes(x = Species, Sepal.Width)) +
