...
In this post, I will share with you some useful tips for adjusting the axis scales for individual panels when you're using ggplot facets. I have been dealing with this problem several times before (and I guess you too!), and so I think this would be a great topic to write about. Continue reading and you will surely get something out of it!  


## The problem

We will be using the `airquality` dataset throughout the post. This dataset contains the daily measurements of four environmental variables, including ozone concentration, solar radiance, temperature, and wind speed, in New York from May to September 1973. 
...
Hmm, the y-axes of the panels look fine, but what if we want to make them prettier (e.g., change the axis ranges or tick breaks)? Can we modify the axis scales for the individual facets to our heart's content? Of course we can. Keep reading!


## The solutions
Customizing the axis scales in ggplot facets is definitely not an easy task, and sometimes it requires a lot of trial and error. There are two main ways to do so:  

<span style="display: block; margin-top: 0px; margin-left: 10px"> **(1) Manual fix: adjusting the axis scales step by step**</span>
<span style="display: block; margin-top: 5px; margin-left: 10px"> **(2) Quick fix: setting the axis scales using extension package**</span>

I'll go through them in more detail in the following section.

### (1) Manual fix
The basic idea of the "manual fix" method is to identify the individual facets and apply specific scale arguments to each of them (). This method includes three steps: **adjust the limits**, **adjust the breaks**, and **adjust the labels**.
