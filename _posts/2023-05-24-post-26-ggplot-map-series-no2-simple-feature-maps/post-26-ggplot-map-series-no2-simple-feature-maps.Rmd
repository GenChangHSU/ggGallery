...
Let's start by preparing the data we need for making the map. The data files we'll be using include:

1. A shapefile of California state boundary from [Natural Earth](https://www.naturalearthdata.com/downloads/10m-cultural-vectors/)  
2. A shapefile of California national park boundaries from [National Park Service Open Data Portal](https://public-nps.opendata.arcgis.com/datasets/nps-boundary-1/explore?filters=eyJTVEFURSI6WyJDQSJdfQ%3D%3D&location=36.531656%2C-116.748610%2C5.59) 
3. A CSV file of the number of visitors in each park in 2022 from [National Park Service Stats Report Viewer](https://irma.nps.gov/Stats/SSRSReports/National%20Reports/Visitation%20By%20State%20and%20By%20Park%20(2017%20-%20Last%20Calendar%20Year))

The shapefiles are read into R using the function `st_read()` from the package `sf`, which reads the shapefiles as an "sf" object. 
