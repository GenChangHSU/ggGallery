...
The map is here, but doesn't really look pleasant. So let's polish it now!

### Polish the map

We’re going to make some changes to the map:

1. Change the background and park colors
2. Adjust the legend appearance
3. Add park name labels
4. Add a title, text annotations, and a logo
...
library(ggtext)
library(grid)
library(ggsflabel)  # add labels from sf objects
...
CA_park_map <- ggplot() + 
  ...
  # text annotation
  geom_textbox(aes(x = -114.75, y = 40, label = "This is a map of the number of visitors in the national parks in California in 2022. The top three most visited parks are **Golden Gate National Recreation Area**, **Yosemite National Park**, and **Joshua Tree National Park**."), 
...
To recap, in this post we learned how to make a ggplot map from external shapefiles. The key process is to read the shapefiles into R as sf objects, make sure their coordinate reference systems are compatible, and pass the sf object to `geom_sf()` for plotting. Finally, a bit of polishing would be great and bring the map to the next level!
