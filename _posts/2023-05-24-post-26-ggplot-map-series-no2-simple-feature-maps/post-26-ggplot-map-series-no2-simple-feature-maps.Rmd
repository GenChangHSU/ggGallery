...
The shapefiles are read into R as an "sf" object using the function `st_read()` from the package `sf`.
...
Park_visitors <- read_csv("CA_Park_Visitation.csv") %>% 
...
Now we have the data in R, the next step is to process them for plotting. There are two things here. First, we'll join the park boundary and park visitor data so that later we can map the numbers of visitors in the parks to different colors. Second, we'll check the coordinate reference systems (crs) of the two data layers (state boundary and park boundaries) to ensure that they can be drawn in the same map.
...
In this example, the two layers have different crs (EPSG code 4326 vs. 3857), so we used `st_transform()` to change the crs of the park boundary data to the one same as that of the state boundary data.
...
The map is out, but doesn't look so pleasant. So let's polish it now!

### Polish the map

1. Change the background color
2. Change the park colors
3. Modify the legend
4. Add text annotations and NPS logo
5. Add park labels
...
CA_park_map <- ggplot() + 
  ...
  geom_sf_text_repel(data = Park_boundary_visitor, aes(label = str_wrap(Park, width = 25)), color = "white", size = 2.75, family = "sans", fontface = "bold", segment.size = 0.2, lineheight = 0.5, vjust = 0.5, hjust = 0.5, max.overlaps = Inf, force = 3) + 
  geom_richtext(aes(x = -118.75, y = 40, label = "<img src='NPS_logo.PNG' width='27.5' />"), fill = NA, label.color = NA, label.padding = unit(rep(0, 4), "pt")) + 
  geom_richtext(aes(x = -115.5, y = 41.5, label = "What Are the Most Visited <br> National Parks in California?"), color = "white", family = "Lora", fontface = "bold.italic", size = 6.5, lineheight = 0.75, fill = NA, label.color = NA, label.padding = unit(rep(0, 4), "pt")) +
  geom_textbox(aes(x = -114.75, y = 40, label = "This is a map of the number of visitors in the national parks in California in 2022. The top three most visited parks are **Golden Gate National Recreation Area**, **Yosemite National Park**, and **Joshua Tree National Park**."), color = "white", family = "Lora", size = 3.75, width = unit(1.8, "in"), lineheight = 0.6, fill = NA, box.color = NA) + 
  ...
  theme(plot.background = element_rect(fill = "grey40", color = "grey40"),
        ...
        legend.text = element_text(size = 9, family = "Lora", lineheight = 0.5, color = "white", margin = margin(t = -2)),
