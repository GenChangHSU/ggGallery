...
diamonds_long <- diamonds_wide %>%
  mutate(alluvium = row_number()) %>%  # add an ID for each alluvium 
  pivot_longer(cols = -c(count, alluvium), names_to = "variable", values_to = "level") %>% 
  mutate(level = factor(level, levels = c("Large", "Small", "Ideal", "Premium", "Very Good", "Good", "Fair", "High", "Mid", "Low")))  # adjust the level order for plotting
...
ggplot(diamonds_long, aes(x = variable, y = count, stratum = level, alluvium = alluvium)) +
