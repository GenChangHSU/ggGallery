...
<span id = "bb" style="display: block; margin-top: 5px; margin-left: 50px">The bars are positioned along the horizontal axis using the arguments `aes(axis1 = , axis2 = , ...)`</span>

<span id = "cc" style="display: block; margin-top: 5px; margin-left: 50px">The function `geom_stratum()` will create a new internal variable "`stratum`" that can be used to label the bars by specifying `aes(label = after_stat(stratum))` in `geom_text()`</span>

<span id = "dd" style="display: block; margin-top: 5px; margin-left: 50px">The alluvia are colored by price</span>
...
As mentioned earlier, `ggalluvial` also works with data in long format. So let's convert the data now. Note that we need to add an ID column to the dataframe before melting it into long format. These ID's will tell the functions the flow connections between adjacent bars (each ID represents a flow stream across all the bars).
...
Do you notice the difference from the previous alluvial diagrams created with wide format data? You're right! The bars and flows have their own colors now! With this method, we are able to color the bars and flows between adjacent bars by the corresponding levels.
...
As mentioned in the beginning, alluvial diagrams are best for visualizing the relationships between categorical variables. Additionally, if the bars along the axis are ordered by time, distance, or some kind of environmental gradients, then they can help reveal trends.
