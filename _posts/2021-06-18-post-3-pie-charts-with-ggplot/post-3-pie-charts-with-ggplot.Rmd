...
By the way, because the last level (in this example "cyl 8") is at the bottom of the barplot and will go first in the pie chart, we need to reverse the order of the levels in our original dataframe to match the order of the stacks in the plot.

```{r, fig.width = 5, fig.height = 3, out.width = "70%"}
# Compute the midpoints of the arcs
label_pos <- n_cyl %>%
  arrange(desc(cyl)) %>%  # Reverse the order of the levels
...
Additionally, you might want to annotate the slices with proportions. We will create a new dataframe and use `geom_text()` to map the labels to the slices. The x positions are just an arbitrary "x" (same as the one in `geom_bar()`); the y positions are the midpoints of the arcs.
...
A even simpler alternative is to map the labels to the stacks using `geom_text()` and center-align their vertical positions by specifying the argument `position_stack(vjust = 0.5)`:      
...
## Some variants
Now, let's see some variants of pie charts:
